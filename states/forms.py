...
class AddNewAdmin(StatesGroup):
    wait_for_admin_ad = State()


class MassSending(StatesGroup):
    wait_rom_content_msg = State()


@dp.message(MassSending.wait_rom_content_msg)
async def get_new_admin_id(msg: types.Message, state: FSMContext):
    if msg.text == "Скасувати":
        await state.clear()
        await msg.answer("Розсилку скасовано", reply_markup=reply.admin_main())
    else:
        await send.global_mass_sending(msg.text, msg.from_user.id)
        await state.clear()


@dp.message(AddNewAdmin.wait_for_admin_ad)
async def get_new_admin_id(msg: types.Message, state: FSMContext):
    print("Admin ID", int(msg.text))
